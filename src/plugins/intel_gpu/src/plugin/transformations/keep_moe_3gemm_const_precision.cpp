...
KeepMOE3GemmConstPrecision::KeepMOE3GemmConstPrecision() {
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS,
        ...
        if (pattern_map.count(moe_softmax_m) > 0) {
            moe_3gemm_fused_compressed = ov::as_type_ptr<ov::intel_gpu::op::MOE3GemmFusedCompressed>(pattern_map.at(moe_softmax_m).get_node_shared_ptr());
        } else if (pattern_map.count(moe_sigmoid_m) > 0) {
            moe_3gemm_fused_compressed = ov::as_type_ptr<ov::intel_gpu::op::MOE3GemmFusedCompressed>(pattern_map.at(moe_sigmoid_m).get_node_shared_ptr());
        }
