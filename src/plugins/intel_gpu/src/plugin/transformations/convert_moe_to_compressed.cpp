...
ConvertMOEToMOECompressed::ConvertMOEToMOECompressed(bool is_pa) {
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) {
        ...
        if (moe->get_config().expert_type == ov::op::internal::MOE::Expert_type::GEMM3_SWIGLU) {
            ...
            std::shared_ptr<ov::Node> moe_output = moe_compressed;
            if (moe->get_output_element_type(0) != moe_compressed->get_output_element_type(0)) {
                ...
                auto convert = std::make_shared<ov::op::v0::Convert>(moe_compressed, moe->get_output_element_type(0));
                convert->set_friendly_name(moe->get_friendly_name());
                ov::copy_runtime_info(moe, convert);
                moe_output = convert;
            }
            ov::replace_node(moe, moe_output);
